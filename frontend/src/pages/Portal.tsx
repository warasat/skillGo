  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // 👇 Graceful handling if backend blocks access
        if (error.response?.status === 403) {
          console.warn("User not allowed to access instructor dashboard");
        } else {
...
const Portal = () => {
  return (
    <PortalLayout>
      <RoleGuard allowedRoles={["instructor"]} fallback={<LearnerDashboard />}>
        <InstructorDashboard />
      </RoleGuard>
    </PortalLayout>
  );
